...
@register_transform(name="MeshRandomAffineTransform")
class MeshRandomAffineTransform(AbstractKeypointTransform):
    """
    ...
    def apply_to_sample(self, sample: MeshEstimationSample) -> MeshEstimationSample:
        ...
        if random.random() < self.prob:
            ...
            sample.rotation_matrix = self.apply_to_matrix(sample.rotation_matrix, angle)
            sample.vertices_2d = self.apply_to_keypoints(sample.vertices_2d, mat_output, sample.image.shape[:2])
    ...
    @classmethod
    def apply_to_matrix(cls, rotation_matrix: np.ndarray, angle: float) -> np.ndarray:
        """
